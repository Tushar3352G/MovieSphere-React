...
const TvShows = () => {
  ...
  useEffect(() => {
    document.title = `MovieSphere | Tv Shows`;
    refresHandler();
  }, [Category]);
    ...
    <div className="w-full min-h-screen bg-zinc-900 text-white">
      <div className="flex items-center justify-between px-8 py-8">
        <h2
          ...
          onClick={() => Navigate("/")}
        >
          <i className="ri-arrow-left-line font-medium text-xl"></i>Tv Shows
        </h2>
      ...
      <div className="py-12">
        ...
        >
          {show.map((item, index) => (
            <Card Category={Category} key={item.id * index} data={item} title='Tv' />
          ))}
        </InfiniteScroll>
      </div>
    </div>
